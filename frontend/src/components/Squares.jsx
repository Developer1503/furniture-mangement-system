...
const Squares = ({
  ...
  const [squareSize, setSquareSize] = useState(50); // Square size set to 50px
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      const newSquareSize = Math.min(canvas.width, canvas.height) / 10; // Adjust square size based on screen size
      setSquareSize(newSquareSize);
      numSquaresX.current = Math.ceil(canvas.width / newSquareSize) + 1;
      numSquaresY.current = Math.ceil(canvas.height / newSquareSize) + 1;
    };
